...
public interface ChamCongRepository extends JpaRepository<ChamCong, Long> {
    ...
    @Query(value = "SELECT * FROM cham_cong WHERE ngay_truc BETWEEN CURDATE() - INTERVAL 7 DAY AND CURDATE() AND nhan_vien_id =?1", nativeQuery = true)
    List<ChamCong> findChamCongsByNhanVienOnWeek(Long nhanVienId);

    @Query(value = "SELECT * FROM computerlab.cham_cong WHERE nhan_vien_id =?3 AND (ngay_truc BETWEEN ?1 AND ?2)", nativeQuery = true)
